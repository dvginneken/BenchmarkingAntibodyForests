...
VDJ_clonal_expansion <- function(VDJ,
  ...
  for (i in unique(VDJ[,group.by])){
    ...
    if(color.by %in% c("isotype", "VDJ_cgene")){
      #Annotate as isotype
      ...
      if(sub.type == F){
        ...
        group_VDJ$cl <- gsub("IGHA1", "IgA", group_VDJ$cl)
        ...
        group_VDJ$cl <- gsub("IGHG1", "IgG", group_VDJ$cl)
        group_VDJ$cl <- gsub("IGHG2", "IgG", group_VDJ$cl)
        group_VDJ$cl <- gsub("IGHG3", "IgG", group_VDJ$cl)
        group_VDJ$cl <- gsub("IGHM", "IgM", group_VDJ$cl)
      ...
      if(is.null(colors)){colors <- Hue_Pal(length(unique_cl_values))}
      cl_colors <- setNames(colors, unique_cl_values)
    }
    ...
    p <- ggplot(group_VDJ, aes(x=factor(clonotype_id, levels = clonotypes), y=cell, fill=cl)) +
      ...
      scale_fill_manual(name = color.by, values = cl_colors) +
      #scale_x_discrete(labels=1:clones) +
      scale_x_discrete(labels=c(rep("",9),10,rep("",9),20,rep("",9),30,rep("",9),40,rep("",9),50)) +
      ggtitle(i)
    # pdf(file = paste0(out.dir, "/ClonalExpansion_",i,".pdf"),
    #    width = 8,
    #    height = 6)
    print(p)
    # dev.off()
